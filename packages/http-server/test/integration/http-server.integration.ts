...
describe('HttpServer (integration)', () => {
  ...
  function givenHttpServer(
    ...
    return new DefaultHttpServerFactory().create(
      requestListener,
      serverOptions,
    );
  }
  ...
  class Http2Server extends DefaultHttpServer {
    protected createServer() {
      if (
        this._protocol === 'http2' ||
        this.serverOptions.hasOwnProperty('spdy')
      ) {
        this.createHttp2();
      } else {
        super.createServer();
    ...
    protected createHttp2() {
      this._server = spdy.createServer(
    ...
    public get protocol() {
      return 'https' as 'https';
    }
  }

  class Http2ServerFactory implements HttpServerFactory {
    create(
      requestListener: RequestListener,
      serverOptions?: HttpServerOptions,
    ): HttpServer {
      return new Http2Server(requestListener, serverOptions);
    }
  }
  ...
  function givenHttp2Server(options: Http2Options): HttpServer {
    ...
    options.key = fs.readFileSync(keyPath);
    options.cert = fs.readFileSync(certPath);
    return new Http2ServerFactory().create(dummyRequestHandler, options);
  }
});
